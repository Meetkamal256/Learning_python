...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args: List[str]):
        ...
        # check if class name doesn't exist in the program
            ...
            for arg in args[1:]:  # for loop that iterates over a slice of the list args
                if '=' in arg:
                    ...
                    try:
                        ...
                        try:
                            # if it can't be converted to an integer, try converting to a float
                            value = float(value)
                        except ValueError:
                            # if it's neither an integer nor a float, strip quotes and unescape quotes
                            value = value.strip('"').replace('\\"', '"')
                            # set the attribute on the new instance with the key and value
